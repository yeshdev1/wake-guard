...
- **Coverage gap — no step here produces the `.unavailable` card, so nobody has ever looked at it.** Steps 1–4 produce a normal card, 5 a *slow* card (slow is not the same as past the deadline), 6 the empty-data card, 7 a rebuild. None of them renders "We couldn't check your sleep readiness just now." on a device, and `ScreenshotTourUITests` does not reach it either — the tour's `UnavailableSleepQuery` returns `[]` rather than throwing, so it lands in `.assessed`. That branch's copy is verified by unit tests only, and its a11y identifiers `readinessUnavailableReason` / `readinessLoading` are referenced by nothing anywhere. **This is a known hole, not something to attempt on device:** forcing it requires a >15s or erroring `HKSampleQuery`, which no user action reliably produces. The cheap close is one launch argument wiring a never-answering sleep query plus one tour assertion (the outstanding **M4** gap); note also that `make test-ui` runs in neither `ci` nor `ci-fast`. Do not record a Fail for not seeing this card.
